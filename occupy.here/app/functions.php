...
function blink_leds($message) {
  $led = '/sys/class/leds/tp-link:green:';
  if (empty($message)) {
    file_put_contents("{$led}wps/message", ' ');
    file_put_contents("{$led}3g/trigger", 'heartbeat');  
    file_put_contents("{$led}wlan/trigger", 'heartbeat');
    file_put_contents("{$led}lan/trigger", 'heartbeat');
  } else {
    file_put_contents("{$led}wps/message", $message);
    file_put_contents("{$led}3g/trigger", 'none');
    file_put_contents("{$led}wlan/trigger", 'none');
    file_put_contents("{$led}lan/trigger", 'none');
  }
}
...
function show_attachment($post) {
  ...
  if (preg_match('/\.(jpe?g|gif|png|bmp|tiff?)$/i', $attachment->path)) {
    ...
    return true;
  } else if (preg_match('/\.mp4$/i', $attachment->path)) {
    echo "<div id=\"mp4\">
      <video controls>
...
function check_first_run() {
  if (file_exists(GRID_DIR . '/data/.revision')) {
    return;
  }
  file_put_contents(GRID_DIR . '/data/.revision', REVISION);
  if (!file_exists('/bin/opkg')) {
    return;
  }
  if (!function_exists('gettext')) {
    $libintl_full = glob(GRID_DIR . '/bootstrap/step1/libintl-full*');
    $php5_mod_gettext = glob(GRID_DIR . '/bootstrap/step4/php5-mod-gettext*');
    if (!empty($libintl_full) && !empty($php5_mod_gettext)) {
      $libintl_full = $libintl_full[0];
      $php5_mod_gettext = $php5_mod_gettext[0];
      exec("opkg install -d opt $libintl_full");
      exec("opkg install -d opt $php5_mod_gettext");
      $restart_lighttpd = true;
    }
  }
  if (!function_exists('mb_substr')) {
    $php5_mod_mbstring = glob(GRID_DIR . '/bootstrap/step4/php5-mod-mbstring*');
    if (!empty($php5_mod_mbstring)) {
      $php5_mod_mbstring = $php5_mod_mbstring[0];
      exec("opkg install -d opt $php5_mod_mbstring");
      $restart_lighttpd = true;
    }
  }
  if (!empty($restart_lighttpd)) {
    copy(GRID_DIR . '/bootstrap/step6/php.ini', '/etc/php.ini');
    exec('/etc/init.d/lighttpd restart');
  }
  copy(GRID_DIR . '/bootstrap/step6/banner', '/etc/banner');
}

function get_current_ssid() {
  if (file_exists('/etc/config/wireless')) {
    exec('uci get wireless.@wifi-iface[0].ssid', $output);
